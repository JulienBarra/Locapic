...
import { addPlace, getPlace } from '../reducers/user';
...
export default function MapScreen() {
  ...
  useEffect(() => {
    fetch(`http://10.9.0.222:3000/places/${user.nickname}`)
      .then((response) => response.json())
      .then((data) => {
        data.result && dispatch(getPlace(data.places));
      });
  }, []);
  ...
  const handleNewPlace = () => {
    fetch('http://10.9.0.222:3000/places', {
      method: 'POST',
